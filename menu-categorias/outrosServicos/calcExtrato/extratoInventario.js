...
document.getElementById("calcularButton").addEventListener("click", calculate);
...
function calculate() {
  var valorSemAdd = 27.27;

  // Obtém o número de folhas do campo de entrada
  var numeroFolhas = document.getElementById("numeroFolhas").value || 1;
  valorSemAdd *= numeroFolhas; // Multiplica o valor pela quantidade de folhas

  var fetj = valorSemAdd * 0.2;
  fetj = Math.floor(fetj * 100) / 100;

  var funperj = valorSemAdd * 0.05;
  funperj = Math.floor(funperj * 100) / 100;

  var funarpen = valorSemAdd * 0.06;
  funarpen = Math.floor(funarpen * 100) / 100;
  ...
  var resultText =
    ...
    '<tr><td>Extrato de Inventário</td><td>' + formatarValorEmReais(valorSemAdd) + '</td></tr>' +
...
function exportarParaExcel() {
  ...
  XLSX.writeFile(workbook, 'Resumo_extrato_inventario.xlsx');
}
